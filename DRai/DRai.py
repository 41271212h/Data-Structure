...
OUTPUT_FIELDS = ["Rules", "History", "Fun Fact", "Origin"]

ITEMS = [
    "Kinds of Sports",            # 是否提及這是球類、水上、田徑、格鬥等運動分類
    "Characteristic",            # 是否描述運動的獨特魅力或特點（速度快、技巧性強、團隊合作等）
    "Demand of Fitness",            # 是否提及運動對體力或耐力的需求（強度高、輕鬆等）
    "Require of Skills",            # 是否提及運動技巧或技術面的要求
    "Fit of Ages",            # 是否說明適合什麼年齡層或特定族群（小孩、成人、老人）
    "Safety",              # 是否提及運動的危險性或安全注意事項
    "Benefits of Health",            # 是否提及此運動對身體健康或心理健康的好處
    "Popularity and Tournament",      # 是否提到該運動的全球/地區流行程度或有代表性的賽事
    "History and Origin",      # 是否提及運動的歷史或發源地
]
...
def process_batch_dialogue(client, dialogues: list, delimiter="-----"):
    ...
    """
    prompt = (
        "你是一位運動專家，請根據以下編碼規則評估對方是否了解運動與其相關規則，\n"
        + "\n".join(ITEMS) +
