...
async def process_user_diary(socketio: SocketIO, user_id, user_entries: pd.DataFrame):
    ...
    prompt = (
        ...
        f"日記內容（僅顯示前 5 筆）：\n{prompt_records}\n\n"
        "請仔細分析上述資料，找出用戶目前學習語言的困難與弱點，並根據你的分析生成一段對於學習日文的建議，內容必須包含：\n"
        "1. 當前學習情況的詳細分析\n"
        "2. 實際可行的學習方案建議\n"
        "3. AI 助理如何提供個性化互動建議\n\n"
        "請注意：請僅生成全新內容，不要重複上述提示。請在回覆最後直接輸出最終建議，格式必須以『我的建議是：』開頭。"
    )
    ...
    try:
        for _ in range(6):  # 最多 6 輪互動
            for agent in agents:
                ...
                if "最終建議：" in response:
                    final_recommendation = response.split("我的建議是：")[-1].strip()
